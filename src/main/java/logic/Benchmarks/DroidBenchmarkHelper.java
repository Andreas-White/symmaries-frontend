...
public class DroidBenchmarkHelper extends SymmariesResultsHelper {
	...
	private static Tool tool = new Tool(inputPath, outputPath, inputPath);
	...
	private void extractExpectedResultFromATestCase(File javaFile) {
		...
		String applicatioName = javaFile.getAbsolutePath().replaceAll(inputPath, "");
	...
	private void processSCGSResults() throws FileNotFoundException {
		String experimentsDirectory = outputPath + "Android/";
		extractSymmariesStatistics(experimentsDirectory, outputPath);
		exportApplicationsStatistics(outputPath, outputPath + "Misc/DroidBench.txt");

		extractExpectedResultsFromMicrobenchmark(inputPath + "Android/DroidBench-master/eclipse-project");
