...
public class SourceSinkHelper {
	...
	/*	boolean returnsSink(SootMethod method, boolean isHigh) {
		if (getSinks(isHigh).get(Utils.XMlReturnValue) != null)
			for (String[] source : getSinks(isHigh).get(Utils.XMlReturnValue))
				if (method.getDeclaringClass().getName().equals(ClassConstant.v(source[0]).toSootType().toString())
						&& method.getName().equals(source[1].split("\\(")[0]))
					return true;
		return false;
